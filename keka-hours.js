...
(function () {
  ...
  function minutesBetween(s, e) {
    ...
    let m =
      (en.hours - st.hours) * 60 +
      (en.minutes - st.minutes);
  ...
  function processLogs() {
    ...
    if (activeStart) {
      ...
      if (st) {
        ...
        const live =
          Math.floor((now - startDate) / 60000);
  ...
  function celebrate() {
    ...
    alert('🏆 8 HOURS ACHIEVED\n\nMajdoori khatam bhai.');
  }
  ...
  function warnTenMinutes(left) {
    ...
    alert(`⚠️ ${left} mins left\n\nFinal castle aa gaya.`);
  }
  ...
  function updateUI() {
    ...
    const pct =
      Math.min(
        100,
        Math.round((total / WORK_MINUTES) * 100)
      );
    ...
    if (data.firstStart) {
      ...
      if (st) {
        ...
        document.getElementById('halfTime').innerText =
          `☀️ Half Day: ${fmtTime(half)}`;

        document.getElementById('fullTime').innerText =
          `🏁 Full Day: ${fmtTime(full)}`;
      }
    }
  ...
  console.log(
    '%c🎮 KEKA GRIND TRACKER LOADED (BUG FIXED)',
